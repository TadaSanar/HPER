...
#%%
###############################################################################
def hper_bo(rounds = 10, starting_points = 2, batch_size = 1, acquisition_function = 'EI_DFT', beta = 0.025, no_plots = False, data_fusion_property = 'dft'):
    '''
    Note: Option data_fusion_property really does something only if EI_DFT is chosen.
    The valid options for data_fusion_property are 'dft', 'visual', 'cutoff'.
    Cutoff poorly implemented aT THE MOMENT!
    '''
    ...
    # Let's choose one of the above as the data fusion constraint.
    if data_fusion_property == 'dft':
    ...
    elif data_fusion_property == 'visual':
        
        data_fusion_files = visual_files
        data_fusion_variable = visual_variable
        lengthscale = visual_lengthscale
        variance = visual_variance
        #beta = visual_beta
        midpoint = visual_midpoint
        
    elif data_fusion_property == 'cutoff':
        
        data_fusion_files = cutoff_files
        data_fusion_variable = cutoff_variable
        #lengthscale = cutoff_lengthscale
        #variance = cutoff_variance
        #beta = cutoff_beta
        #midpoint = cutoff_midpoint
    ...
    for k in range(rounds):
        ...
        if (k == 0) and (function == True):
            ...
            grid_init = np.array(starting_points)#[[0.33, 0.33, 0.33], [0,0,1]])#1, 0, 0]])#, [0,1,0], [0,0,1], [0.5,0.5,0], [0.5, 0, 0.5], [0, 0.5, 0.5]]) # To do: generalize
