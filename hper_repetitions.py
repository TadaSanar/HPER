...
def repeated_tests(m, starting_point_candidates):#, gt_model_targetprop,
    ...
    c_eig = [0.1]#[0.05, 0.1, 0.25]#, 1]#[0.75]  # Expected information gain.
    # Size of the exclusion zone in percentage points (max. 100)
    c_exclz = [1]#[5, 10, 15]#, 20]#[20]
    # Gradient limit. When the number is higher, the criterion picks less points. 0.05#, 0.07, 0.1, 0.2, 0.5, 0.75
    c_g = list(cg(np.array([0.9])))#list(cg(np.array([0.95, 0.9, 0.8])))#list(cg(np.array([0.6])))

    jitters = [5]

    folder = './Results/20240920/LCB_DF/Test3/Noise000/' # $WRKDIR/Results/ for the server
    #ground_truth = [0.165, 0.04, 0.79] #[0.17, 0.03, 0.80]  # From C2a paper

    bo_params = {'n_repetitions': 1, # Repetitions of the whole BO process.
                 'n_rounds': 5, # Number of rounds in one BO.
                 'n_init': 1, # Number of initial sampling points.
    ...
    indices_of_repeats = [0]#range(bo_params['n_repetitions'])
    #indices_of_repeats = np.arange(10, 20, 1)
