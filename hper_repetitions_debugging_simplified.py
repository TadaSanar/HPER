...
def repeated_tests(m, starting_point_candidates):#, gt_model_targetprop,
    ...
    c_eig = [0.05, 0.1]#[0.05, 0.1, 0.25]#, 1]#[0.75]  # Expected information gain.
    # Size of the exclusion zone in percentage points (max. 100)
    c_exclz = [5, 10]#[5, 10, 15]#, 20]#[20]
    # Gradient limit. When the number is higher, the criterion picks less points. 0.05#, 0.07, 0.1, 0.2, 0.5, 0.75
    c_g = list(cg(np.array([0.8])))#list(cg(np.array([0.95, 0.9, 0.8])))#list(cg(np.array([0.6])))

    jitters = [0.1, 0.5]

    folder = './Results/20240831/Test_ho_quick/' # $WRKDIR/Results/ for the server
    ...
    indices_of_repeats = np.arange(5, 10, 1)
