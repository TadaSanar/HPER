...
def repeated_tests(m, starting_point_candidates):#, gt_model_targetprop,
    ...
    folder = '$WRKDIR/Results/20240808/Test-new-env/Serial/' # $WRKDIR/Results/ for the server
    ...
    if (m > -1):
        ...
        if fetch_old_results == False:
            ...
            for i in range(bo_params['n_repetitions']):
                ...
                # also four times in between if the total number of repetitions
                # is large.
                if (i == (bo_params['n_repetitions']-1)) or (
                        (bo_params['n_repetitions'] > 15) and
                        (np.remainder((i+1),
                                      int(np.floor(bo_params['n_repetitions']/4)))
                         == 0)):
