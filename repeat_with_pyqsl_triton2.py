...
def calc_additional_idx_for_folder(param1, param2):
    ...
    param1_array = cg(np.linspace(0.01, 1, 5))
    param2_array = np.linspace(0, 1, 5)
...
settings.init_points = np.array(np.load('./Source_data/initpts75.npy'))
...
#result = pyqsl.run(task=task, settings=settings, parallelize=False)
#result = pyqsl.run(task=task, settings=settings, sweeps=dict(m=np.array([0,0])), parallelize=False)
...
result.dataset.n_df.mean(dim='indices_of_repeats').plot.line(x='rounds_as_list')
plt.show()
...
# Compare benchmark criteria on all jitters.
for i in range(2):#len(jitters)):
    #jitter = jitters[i]
    result.dataset.optima.mean(dim='indices_of_repeats').sel(
        mat_dim='target', m=i, drop=True).plot.line(x="rounds_as_list")
    plt.show()
for i in range(2):#len(jitters)):
    #jitter = jitters[i]
    result.dataset.regrets.mean(dim='indices_of_repeats').sel(
        m=i, drop=True).plot.line(x="rounds_as_list")
    plt.show()
...
# Hard-coded for selected hyperparam combinations.
result.dataset.regrets.mean(dim='indices_of_repeats').sel(
        c_g=c_g_options[-1], c_eig = c_eig_options[2], drop=True).plot.line(x='rounds_as_list')
result.dataset.regrets.mean(dim='indices_of_repeats').sel(
        c_g=c_g_options[-2], c_eig = c_eig_options[1], drop=True).plot.line(x='rounds_as_list')
result.dataset.regrets.mean(dim='indices_of_repeats').sel(
        c_g=c_g_options[-2], c_eig = c_eig_options[-1], drop=True).plot.line(x='rounds_as_list')
plt.show()
result.dataset.n_df.mean(dim='indices_of_repeats').sel(
